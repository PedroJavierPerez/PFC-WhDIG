...
//    numPag = (Id_evento -1) * 2;
      ...
      location.href= URL_BASE+"Administrador/index/"+numPag;
...
function opcionesFiltrado(){
    ...
    var data = { 
        ...
        $.ajax({
        url:URL_BASE+"Administrador/filtrarEventos",
        type:"POST",
        data: data,
        beforeSend: function() {
            console.log("enviando datos a DB")
        ...
        if(resp === "Fecha no valida"){
           ...
           string += "<a id='"+eventos[i].id+"' href='Administrador/detalles/"+eventos[i].id+"'><article>\n\
